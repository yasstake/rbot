
use rbot_lib::common::{hmac_sign, MarketConfig, MultiMarketMessage, ServerConfig, BLOCK_ON, NOW, SEC};

use rbot_lib::net::{AutoConnectClient, WebSocketClient, WsOpMessage};
use serde_derive::Deserialize;
use serde_derive::Serialize;
use crossbeam_channel::Receiver;
use tokio::task::JoinHandle;

use crate::message::{BybitUserStreamMessage, BybitWsMessage};

use super::config::BybitServerConfig;

const PING_INTERVAL_SEC: i64 = 20;
const SWITCH_INTERVAL_SEC: i64 = 60 * 3; // 3min for test
pub const SYNC_WAIT_RECORDS: i64 = 0; // no overlap

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct BybitWsOpMessage {
    pub op: String,
    pub params: Vec<String>,
    pub id: i64,
}

impl WsOpMessage for BybitWsOpMessage {
    fn new() -> Self {
        BybitWsOpMessage {
            op: "subscribe".to_string(),
            params: vec![],
            id: NOW() % 1000,
        }
    }

    fn add_params(&mut self, params: &Vec<String>) {
        log::debug!("add_params: {:?} / {:?}", self.params, params);
        self.params.extend(params.clone());
    }

    fn make_message(&self) -> Vec<String> {
        let mut messages: Vec<String> = vec![];
        for arg in &self.params {
            let m = BybitWsOpMessage {
                op: "subscribe".to_string(),
                params: vec![arg.clone()],
                id: NOW() % 1000,
            };
            messages.push(m.to_string());
        }

        messages
    }

    fn to_string(&self) -> String {
        serde_json::to_string(self).unwrap()
    }
}


pub struct BybitWsClient {
    ws: WebSocketClient<BybitServerConfig, BybitWsOpMessage>,
    handle: Option<JoinHandle<()>>,
}

impl BybitWsClient {
    pub fn new(server: &BybitServerConfig, url: &String, config: &MarketConfig, init_fn: Option<fn(&BybitServerConfig) -> String>, sync_wait_rec: i64) -> Self {
        BLOCK_ON(
            async {
                BybitWsClient::async_new(server, url, config, init_fn, sync_wait_rec).await
            }
        )
    }

    pub async fn async_new(server: &BybitServerConfig, url: &String, config: &MarketConfig, init_fn: Option<fn(&BybitServerConfig) -> String>, sync_wait_rec: i64) -> Self {
        let ws = WebSocketClient::new(
            server,
            config,
            &url,
            config.public_subscribe_channel.clone(),
            PING_INTERVAL_SEC,
            SWITCH_INTERVAL_SEC,
            sync_wait_rec,
            Some(make_auth_message),
            None,
        )
        .await;

        Self {
            ws: ws,
            handle: None,
        }
    }

    pub fn connect(&mut self) {
        BLOCK_ON(async { 
            self.ws.connect(|message|{
                let m = serde_json::from_str::<BybitWsMessage>(&message);

                if m.is_err() {
                    log::warn!("Error in serde_json::from_str: {:?}", message);
                    println!("ERR: {:?}", message);
                }

                let m = m.unwrap();

                return m.into();
            }).await
        })
    }

    pub fn open_channel(&mut self) -> Receiver<MultiMarketMessage> {
        BLOCK_ON(async {
            self.ws.open_channel().await
        })
    }
}

pub fn make_auth_message(server: &BybitServerConfig) -> String {
    let api_key = server.get_api_key();
    let secret_key = server.get_api_secret();
    let time_stamp = (NOW() / 1_000) + 1_000 * 10; // 10 seconds in the future
    let sign = hmac_sign(&secret_key, &format!("GET/realtime{}", time_stamp));

    let message = BybitWsOpMessage {
        op: "auth".to_string(),
        params: vec![api_key, time_stamp.to_string(), sign],
        id: NOW() % 1_000,
    };

    message.to_string()
}

/*
pub async fn listen_userdata_stream<F>(
    config: &BybitServerConfig,
    mut f: F,
) -> tokio::task::JoinHandle<()>
where
    F: FnMut(BybitUserStreamMessage) + Send + 'static,
{
    let url = config.private_ws.clone();

    let mut websocket: AutoConnectClient<BybitServerConfig, BybitWsOpMessage> =
        AutoConnectClient::new(

            config,
            &url,
            // Arc::new(RwLock::new(message)),
            PING_INTERVAL_SEC,
            SWITCH_INTERVAL_SEC,
            SYNC_RECORDS,
            Some(make_auth_message),
        );

    websocket
        .subscribe(&vec![
            "execution".to_string(),
            "order".to_string(),
            "wallet".to_string(),
        ])
        .await;

    websocket.connect().await;

    let handle = tokio::task::spawn(async move {
        loop {
            let msg = websocket.receive_text().await;
            if msg.is_err() {
                log::warn!("Error in websocket.receive_message: {:?}", msg);
                continue;
            }
            let msg = msg.unwrap();
            log::debug!("raw msg: {}", msg);
            let msg = serde_json::from_str::<BybitUserStreamMessage>(msg.as_str());
            if msg.is_err() {
                log::warn!("Error in serde_json::from_str: {:?}", msg);
                continue;
            }
            let msg = msg.unwrap();
            f(msg);
        }
    });

    handle
}

*/

/*
#[cfg(test)]
mod test_ws {
    use crate::common::init_debug_log;

    #[test]
    fn test_make_auth_message() {
        use crate::exchange::bybit::config::BybitServerConfig;
        let config = BybitServerConfig::new(false);
        let msg = super::make_auth_message(&config);
        println!("{}", msg);
    }

    #[test]
    fn test_connect() {
        use crate::exchange::bybit::config::BybitServerConfig;
        use crate::exchange::bybit::ws::listen_userdata_stream;
        let config = BybitServerConfig::new(true);

        init_debug_log();

        listen_userdata_stream(&config, |msg| {
            println!("{:?}", msg);
        });

        std::thread::sleep(std::time::Duration::from_secs(5));
    }
}

/*
TODO: reconnet ws when disconnected.
[2024-01-20T04:55:43Z WARN  rbot::exchange::bybit::ws] Error in websocket.receive_message: Err("Disconnected wss://stream-testnet.bybit.com/v5/private: Trying to work with closed connection")
[2024-01-20T04:55:43Z ERROR rbot::exchange::ws] Disconnected from server
*/

*/

#[cfg(test)]
mod bybit_ws_test {
    use rbot_lib::common::ServerConfig;
    use rbot_lib::common::
    {init_debug_log, NOW, SEC};
    use crate::config::BybitServerConfig;
    use crate::ws::BybitWsClient;
    use crate::ws::make_auth_message;
    use crate::ws::BybitWsOpMessage;
    use crate::message::BybitUserStreamMessage;
    use crate::BybitConfig;
    use std::thread::sleep;
    use std::time::Duration;

    #[test]
    fn test_make_auth_message() {
        let config = BybitServerConfig::new(true);
        let msg = make_auth_message(&config);
        println!("{}", msg);
    }

    #[test]
    fn test_connect() {
        let server = BybitServerConfig::new(true);
        let config = BybitConfig::BTCUSDT();
        let url = server.get_public_ws_server();

        let mut ws = BybitWsClient::new(&server, &url, &config, None, 0);
        ws.connect();

        let receiver = ws.open_channel();

        for i in 0..10 {
            let msg = receiver.recv().unwrap();
            println!("{:?}", msg);
        }

        sleep(Duration::from_secs(5));
    }

    #[test]
    fn test_open_channel() {
        let server = BybitServerConfig::new(true);
        let config = BybitConfig::BTCUSDT();
        let url = server.get_public_ws_server();        
        let mut ws = BybitWsClient::new(&server, &url, &config, None, 0);
        ws.connect();
        ws.open_channel();
        sleep(Duration::from_secs(5));
    }
}

